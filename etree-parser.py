...
>>>>>>> 212fc3f261c2ff25021e9e57afc78a9ca31e375e
xml_line = ''
...
"""
for val in session.query(device.name,\
    device_maker.name,\
    dev_to_validation.driver_name,\
    dev_to_validation.driver_ver,\
    server.name,\
    server_vendor.name,\
    releases.name).\
    filter(validation.server_id == server.id).\
    filter(validation.release_id == releases.id).\
    filter(server.server_vendor_id == server_vendor.id).\
    filter(dev_to_validation.validation_id == validation.id).\
    filter(device.id == dev_to_validation.device_id).\
    filter(device.device_maker_id == device_maker.id):
    print val
"""

# check for argument - file to parse
if len(sys.argv) < 2:
...
for raidcnt in raids:
    ...
    #device_maker_obj = Device_maker(name = nic_vendor.text)
    #device_obj = Device(name = nic_name.text, type = 'raid', maker = device_maker_obj)
    #session.add(device_maker_obj)
    #session.add(device_obj)
    #session.commit()
